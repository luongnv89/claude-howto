...
**Швидкий старт**: Попросіть Claude створити субагента для вас («створи субагента, який перевіряє безпеку») або додайте файл `.claude/agents/<name>.md` безпосередньо — див. [Управління субагентами](#управління-субагентами) нижче.
...
> **Примітка:** Інтерактивний майстер `/agents` видалено у v2.1.198, а починаючи з v2.1.281 `/agents` більше не з'являється в меню `/` чи в `/help`. Попросіть Claude створити субагентів або керувати ними (наприклад, «створи субагента, який перевіряє безпеку») або додайте файл `.claude/agents/<name>.md` безпосередньо.

```text
Створи субагента, який перевіряє код на вразливості безпеки.
```

Claude запише файл `.claude/agents/<name>.md` за вас і підбере доречний frontmatter (інструменти, модель, опис). Потім ви можете доопрацювати файл вручну або попросити Claude його змінити.
...
| `Ctrl+X` `Ctrl+K` | Зупинити всіх запущених фонових субагентів (натисніть двічі протягом 3 секунд для підтвердження; це замінило `Ctrl+F`) |
...
```text
Створи субагента рівня проєкту, який запускає тести та виправляє збої.
Надай йому доступ до Bash, Read, Edit і Grep.
```

Claude запише `.claude/agents/<name>.md` з відповідним frontmatter. Перегляньте згенерований файл, а потім використовуйте його.
...
Після встановлення переконайтесь, що агенти розпізнаються, переглянувши вміст каталогу:

```bash
ls .claude/agents/
```

Ви також можете запитати Claude, які субагенти доступні в поточній сесії, — він назве вбудованих і власних агентів, яким може делегувати завдання.
